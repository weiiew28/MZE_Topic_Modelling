...
#out_tm is the data format for Mimno online LDA
            ...
            for content in potential_content:
                if content.name=='p' and 'class' in content.attrs and set(content.attrs['class']).intersection(set(paragraph_style))!=set([]):
                    ...
                    if paragraph_count + num_words <= text_limit:
                        ...
                        corpus_file = open('/Users/Wei/Documents/Technique_Study/Coding/LeetCode/MZE/Data/Corpus/{0}.txt'.format(convert_num(count,4)),'w')
                        corpus_file.write(cur_article.encode('utf-8','ignore'))
                        corpus_file.close()
                        paragraph_count = num_words
                        count+=1
                        cur_article = stemmed_text+' '
                ...
                if content.name=='h5' or no_title_indicator:
                    if cur_article!='':
                        ...
                        #out_tm_tsv.writerow([count,'vol{0}book{1}article{2}'.format(actual_vol,actual_book,article_count),cur_article.encode('utf-8','ignore')])
                        corpus_file = open('/Users/Wei/Documents/Technique_Study/Coding/LeetCode/MZE/Data/Corpus/{0}.txt'.format(convert_num(count,4)),'w')
                        corpus_file.write(cur_article.encode('utf-8','ignore'))
                        corpus_file.close()
